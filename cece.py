...
if app_theme == "Light Mode":
    ...
    <style>
        ...
        /* Fix for Light Mode Inputs & Labels */
...
if st.button("🚀 Execute AI Network Optimization", type="primary", use_container_width=True):
    if len(st.session_state.locations_df) < 2:
        ...
        if not is_optimal: st.error("❌ Optimization failed. Loosen capacity limits.")
            ...
            st.balloons()
            st.session_state.history.append({"time": datetime.datetime.now().strftime("%I:%M:%S %p"), "zones": len(df_copy), "wh_placed": len(wh_coords)})
...
if st.session_state.opt_results is not None:
    ...
    with tab1:
        # Fit bounds dynamically to frame all points perfectly
    ...
    with tab4:
        ...
        st.download_button(label="📥 Export Enterprise CSV", data=csv, file_name='gridpoint_plan.csv', mime='text/csv', type="primary")
...
if not st.session_state.history:
    ...
    for run in reversed(st.session_state.history[-5:]):
        ...
        <div class="history-card">
            <div style="font-weight: 800; margin-bottom: 4px;">🕒 {run['time']}</div>
