...
# Pre-hashed passwords generated via stauth.Hasher
credentials = {
    "usernames": {
        "admin": {
            "email": "admin@gridpoint.com",
            "name": "Admin User",
            "password": stauth.Hasher.hash("admin123")  # Replace "admin123" with your password
        },
        "analyst": {
            "email": "analyst@gridpoint.com",
            "name": "Data Analyst",
            "password": stauth.Hasher.hash("analyst123")  # Replace "analyst123" with your password
        }
    }
}
...
if st.session_state["authentication_status"] is False:
    ...
    st.warning("Please enter your credentials to access GRIDPOINT")

elif st.session_state["authentication_status"]:
    # --- AUTHENTICATED SECTION ---
    
    # Sidebar Logout Button & User Greeting
    ...
    # --- OPTIMIZATION ALGORITHM (K-MEDOIDS / ILP) ---
    def solve_warehouse_placement(df, k, cap=None, max_rad=None):
        n = len(df)
        coords = df[['lat', 'lon']].values
        orders = df['daily_orders'].values
        
        # Compute distance matrix between all neighborhoods (n x n)
    ...
    try:
        ...
        # Baseline Comparison: Centralized Single Hub
        ...
        for i, row in df.iterrows():
            ...
            # Draw delivery connection lines
            folium.PolyLine(
