...
if st.session_state.opt_results is not None:
    ...
    with tab1:
        ...
        # Draw service radius around warehouses if enabled
        if enable_radius:
    ...
    with tab2:
        ...
        # Build dynamic column config based on whether capacity is enabled
        cfg = {
            "assigned_wh_id": st.column_config.TextColumn("Warehouse ID"),
            "total_orders_handled": st.column_config.NumberColumn(
                "Total Orders", help="Total daily volume", format="%d 📦"
            ),
            "avg_delivery_dist": st.column_config.NumberColumn(
                "Avg Distance (km)", format="%.2f km"
            ),
        ...
        if capacity_limit:
            ...
            cfg["Capacity Utilized %"] = st.column_config.ProgressColumn(
                "Capacity Usage", help="Percentage of max capacity", format="%f%%", min_value=0, max_value=100
            )
